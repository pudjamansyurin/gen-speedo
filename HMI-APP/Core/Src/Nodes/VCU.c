 ...
 * --------------------------------------------*/
void VCU_Init(void) { memset(&VCU, 0, sizeof(vcu_t)); }
...
void VCU_RX_SwitchControl(can_rx_t *Rx) {
  ...
  HMI1.hbar.d.mode[HBAR_M_PREDICTION] = (d->u8[2] >> 4) & 0x01;
...
void VCU_RX_ModeData(can_rx_t *Rx) {
  ...
  switch (HMI1.hbar.d.mode[HBAR_M_PREDICTION]) {
    case HBAR_M_PREDICTION_RANGE:
      HMI1.hbar.d.prediction = d->u8[6];
      break;
    case HBAR_M_PREDICTION_EFFICIENCY:
      HMI1.hbar.d.prediction = d->u8[7];
      break;
    default:
      break;
  }
