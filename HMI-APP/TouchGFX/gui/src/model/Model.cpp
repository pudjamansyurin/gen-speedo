...
Model::Model() : modelListener(0), ticker(0), indicator(1), indicators{0} {
#ifdef SIMULATOR
  setDefaultData();
...
void Model::tick() {
  ...
  if (HMI1.hbar.listening) {
    if (ticker % 20 == 0) session = !session;
  } else
    session = 1;
  ...
  modelListener->setPredictionMode(HMI1.hbar.d.mode[HBAR_M_PREDICTION],
                                   HMI1.hbar.d.prediction);
...
uint8_t Model::readCurrentIndicator() { return indicator; }

uint8_t Model::readIndicatorState(uint8_t index) { return indicators[index]; }
...
void Model::swipeIndicator() {
  ...
  // fall-back
  if (!found) indicator = INDICATOR_NONE;
}
...
void Model::generateRandomData() {
  ...
  if (ticker % 1 == 0) {
    if (VCU.d.mcu.speed >= MCU_SPEED_MAX_KPH)
      VCU.d.mcu.speed = 0;
    else
      VCU.d.mcu.speed += 5;

    if (VCU.d.mcu.discur >= MCU_DISCUR_MAX)
      VCU.d.mcu.discur = 0;
    else
      VCU.d.mcu.discur += 1;
  }

  if (ticker % 10 == 0) HMI1.hbar.d.trip++;

  if (ticker % 20 == 0) {
    if (HMI1.hbar.d.prediction >= 255)
  ...
  if (ticker % 60 == 0) {
    ...
    switch (HMI1.hbar.m) {
      ...
      case HBAR_M_PREDICTION:
        mode = &HMI1.hbar.d.mode[HBAR_M_PREDICTION];
        max = HBAR_M_PREDICTION_MAX - 1;
        break;
      default:
        break;
    }
  ...
  if (ticker % (60 * 60) == 0) generateRandomIndicators();
}
#endif
