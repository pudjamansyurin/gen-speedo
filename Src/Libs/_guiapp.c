...
static void Run_Boot_Animation(void);
static void Set_Boot_Overlay(void);
static void Set_Indicator( GUI_CONST_STORAGE GUI_BITMAP *bg, GUI_CONST_STORAGE GUI_BITMAP *fg, uint16_t x, uint16_t y, uint8_t status,
    uint8_t alpha);
#if USE_HMI_LEFT
static void Set_Left_Sein(uint8_t status);
static void Set_Left_Finger(uint8_t status);
static void Set_Left_Mirror(uint8_t status);
static void Set_Left_Keyless(uint8_t status);
static void Set_Left_Jarum(uint8_t deg, uint16_t x, uint16_t y, uint16_t r, uint16_t h, uint8_t max);
static void Set_Left_Trip(sw_mode_trip_t mode_trip);
#else
static void Set_Right_Sein(uint8_t status);
static void Set_Right_Warning(uint8_t status);
static void Set_Right_Abs(uint8_t status);
static void Set_Right_Temp(uint8_t status);
static void Set_Right_Lamp(uint8_t status);
static void Set_Right_Battery(int8_t status);
#endif
/* Main task ------------------------------------------------------------------*/
void GUI_MainTask(void) {
  ...
  while (1) {
    ...
    if (init) {
      ...
      BSP_Set_Backlight(1);
		...
		if (init || DB_HMI_Mode_Old.mode_trip != DB_HMI_Mode.mode_trip
				...
				|| Mode_Hide_Trip_Old != (DB_HMI_Mode.mode == SW_M_TRIP && DB_HMI_Mode.hide)) {
    ...
    // Mode Report
    {
      ...
      // Mode Report Label
      if (init || DB_HMI_Mode_Old.mode_report != DB_HMI_Mode.mode_report
          || Mode_Hide_Report_Old != (DB_HMI_Mode.mode == SW_M_REPORT && DB_HMI_Mode.hide)) {
    ...
    if (init || DB_HMI_Mode_Old.mode_drive != DB_HMI_Mode.mode_drive
        || Mode_Hide_Drive_Old != (DB_HMI_Mode.mode == SW_M_DRIVE && DB_HMI_Mode.hide)) {
      DB_HMI_Mode_Old.mode_drive = DB_HMI_Mode.mode_drive;
      Mode_Hide_Drive_Old = (DB_HMI_Mode.mode == SW_M_DRIVE && DB_HMI_Mode.hide);

      if (!Mode_Hide_Drive_Old) {
        // if reverse, change color
        if (DB_HMI_Mode.mode_drive == SW_M_DRIVE_R) {
...
static void Set_Default_Data(void) {
  ...
  DB_HMI_Mode.mode = SW_M_TRIP;
  DB_HMI_Mode.hide = 0;
  DB_HMI_Mode.mode_drive = SW_M_DRIVE_E;
  DB_HMI_Mode.mode_trip = SW_M_TRIP_A;
  DB_HMI_Mode.mode_trip_value = 0;
  DB_HMI_Mode.mode_report = SW_M_REPORT_RANGE;
...
static void Set_Indicator(GUI_CONST_STORAGE GUI_BITMAP *bg,
